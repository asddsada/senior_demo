...
def saveWave():
    ...
    fout.close()
    # shutil.move(source_path+'/'+spk + '_save_file.wav', audio_dir+'demo/roc_'+spk+'.wav')
...
def process():
    ...
    os.system("cd "+pipeline_dir+"; ./run_demo_pipeline_test.sh --target-spk "+spk)
...
def getResult():
    ...
    req_data = request.get_json()
    spk = req_data['speaker']
    utt_count = max([ int(f.split('.')[0].split('_')[1])+1 for f in os.listdir(pipeline_dir+'audios_save/test_roc/test_roc') if spk in f ]+[0])
    name = ('roc-%s_%06d' % (spk,utt_count))
    shutil.move(audio_dir+'demo/roc_'+spk+'.wav',pipeline_dir+'audios_save/test_roc/test_roc/'+name+'.wav')
    shutil.copy(pipeline_dir+'exp/scores_train_unlabel_train_gowajee/scores_adapt/test_combined_adapt.trials', result_dir+'roc/'+name+'.trials')
